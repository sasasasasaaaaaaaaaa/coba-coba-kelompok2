            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('privilege.index') }}">{{ $title }}</a></li>
    ...
    <section class="section">
        <div class="card kt-form-card">
            <div class="card-header d-flex align-items-center gap-2">
                ...
                <span>Form Edit Data {{ $title }} untuk Role <u>{{ $role->role }}</u></span>
            </div>
            <div class="card-body">
                @include('include.flash')
                <form class="form form-horizontal" action="{{ route('privilege.update', ['privilege' => $privilege->id, 'id_role' => $role->id]) }}" method="POST">
                    <div class="form-body">
                        @csrf @method('patch')
                        ...
                        <div class="offset-md-3 ps-2 pt-2 d-flex gap-2">
                            ...
                            <a href="{{ route('privilege.index', ['id_role' => $role->id]) }}" class="btn btn-outline-secondary">Batal</a>
                        </div>
                  </div>
                </form>
            </div>
